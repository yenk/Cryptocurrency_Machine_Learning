...
class DataWrangle: 
  ...
  def __create_day_lookup(self): 
    """
    Create a date lookup to extract every record on a daily basis 
    """
    keys = ['Open', 'High', 'Low', 'Volume']
    self.date_lookup_dict = {price_dict['Date']:{k:price_dict[k] for k in keys} for price_dict in self.final_data_manip_list} 
  ...
  def get_year_aggregates(self): 
    ...
    """
    keys = ['Open', 'High', 'Low', 'Volume']
    self.year_dict = {price_dict['Date']:{k:price_dict[k] for k in keys} for price_dict in self.date_lookup_dict}
